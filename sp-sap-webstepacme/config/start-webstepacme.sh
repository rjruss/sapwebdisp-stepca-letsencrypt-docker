...
CERT_NAME="webstepacme"
ACME_PROV="${SP_PROV_NAME}-ACME"
ACME_CERT_CHAIN="/var/tmp/acme-chain.cer"
save_cert_renew_length=0
        ...
        for (( i=1; i<=${1}; i++ )); do
                ...
                if [[ $retVal -eq 0 ]];then
                       # (( stepca_cdur=$(step ca provisioner list --ca-url=${SP_STEP_HOST} --root=${CERT_LOCATION}/stepCA.pem |jq -r '.[0].claims.maxTLSCertDuration | split("h")[0]') ))
                        (( stepca_cdur=$(step ca provisioner list --ca-url="${SP_STEP_HOST}" --root=${CERT_LOCATION}/stepCA.pem |jq -r --arg  pname "$ACME_PROV" '.[]|select(.name == $pname )|.claims.maxTLSCertDuration| split("h")[0]' ) ))
                        (( app_cdur=$(echo "$SP_WEBSTEPACME_CERT_DUR" |sed "s/h//") ))
...
setup_files () {
    ...
    if [[ ! -f /srv/usr/sap/webdisp/manage/control_cert_renew_length ]];then
        (( days=$(echo ${SP_WEBSTEPACME_CERT_DUR//h/})/24))
...
initialise_app () {
	...
	#step ca certificate ${SP_CONTAINER_NAME} ${CERT_LOCATION}/${CERT_NAME}.crt ${CERT_LOCATION}/${CERT_NAME}.key  --not-after ${SP_WEBSTEPACME_CERT_DUR} --san ${SP_CONTAINER_NAME} --san $HOST_FQDN  --provisioner-password-file  <(set +x;echo -n `shared_get_info.sh STEP PW`;set -x)

        
        /srv/usr/sap/commands/acme.sh/acme.sh --set-default-ca --server "${SP_STEP_HOST}/acme/${ACME_PROV}/directory"
...
sign_csr () {
        ...
        hours="$((days*24))h"
	#step ca sign  --not-after "${SP_WEBSTEPACME_CERT_DUR}" "${CSR_LOCATE}" "${SIGNED_CERT}"  --provisioner-password-file  <(set +x;echo -n $(shared_get_info.sh STEP PW);set -x)
        #step ca sign  --provisioner="$ACME_PROV"  --not-after "${hours}" "${CSR_LOCATE}" "${SIGNED_CERT}" 

        /srv/usr/sap/commands/acme.sh/acme.sh --ca-bundle ${CERT_LOCATION}/stepCA.pem --alpn  --signcsr  --csr "${CSR_LOCATE}" --webroot /var/tmp  --force

        #/srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/ca.cer
        #/srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/"${HOST_FQDN}".cer
        # --provisioner-password-file  <(set +x;echo -n $(shared_get_info.sh STEP PW);set -x)
}
...
import_pse_roots () {
	...
	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/SAPSSLC.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/ca.cer
	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/SAPSSLS.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/ca.cer
	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/DOCKERSSL.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/ca.cer

	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/SAPSSLC.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/"${HOST_FQDN}".cer 
	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/SAPSSLS.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/"${HOST_FQDN}".cer
	sapgenpse maintain_pk  -p /srv/usr/sap/webdisp/sec/DOCKERSSL.pse -a /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/"${HOST_FQDN}".cer
...
import_pse_certs () {
	...
	#sapgenpse import_own_cert  -p /srv/usr/sap/webdisp/sec/DOCKERSSL.pse -c ${CERT_LOCATION}/signed-cert-chain.pem
        ...
        cat ${CERT_LOCATION}/stepCA.pem  > ${ACME_CERT_CHAIN}
        cat /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/ca.cer >>${ACME_CERT_CHAIN}
        cat /srv/usr/sap/webdisp/.acme.sh/"${HOST_FQDN}"/"${HOST_FQDN}".cer >> ${ACME_CERT_CHAIN}
        sapgenpse import_own_cert -v -p /srv/usr/sap/webdisp/sec/DOCKERSSL.pse  -c ${ACME_CERT_CHAIN}
}
...
config_file () {
	...
	sed -i "s/ZWEB_HOST_PORT/${WEBSTEPACME_HOST_PORT}/g" /srv/usr/sap/webdisp/sapwebdisp.pfl
        /srv/usr/sap/webdisp/sapgenpse gen_pse -p "${SECUDIR}/DOCKERSSL.pse" -x '' -r "${CSR_LOCATE}" -k GN-dNSName:${HOST_FQDN} 'CN='"${HOST_FQDN}"', DNS='"${HOST_FQDN}"', C=GB'
...
andcheck () {
        ...
        #loginfo "sleep every day until the chosen day to check the cert"
                ...
                do
                        ...
                        if [[ ${save_cert_renew_length} != $(cat /srv/usr/sap/webdisp/manage/control_cert_renew_length ) ]];then
                                days=$(cat /srv/usr/sap/webdisp/manage/control_cert_renew_length )
                                hours="$((days*24))h"
                                step ca provisioner update "${ACME_PROV}"  --admin-provisioner="${SP_PROV_NAME}"  --x509-default-dur="${hours}" -admin-password-file=<(set +x;echo -n $(shared_get_info.sh STEP PW);set -x) --admin-subject=superstep
                                save_cert_renew_length=$(cat /srv/usr/sap/webdisp/manage/control_cert_renew_length )
                        fi
...
if [[ ! -f ${INST_LOCFILE} ]];then
        ...
        if check_step_ca 3 20; then
                ...
                logerr "Failure to connect to step-ca - cant renew certificates but starting WEBSTEPACME and certificates may cause issues "
                startup
                andcheck      
        fi
